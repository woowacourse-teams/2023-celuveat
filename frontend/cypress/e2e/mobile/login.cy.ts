describe('로그인 관련 로직을 테스트 한다.', () => {
  beforeEach(() => {
    ...
    cy.loginGoogleForMobile();
  });

  it('모바일에서 성시경, 소문난성수감자탕 페이지에서 로그인을 한 후 다시 성시경, 소문난성수감자탕 페이지로 돌아 간다.', () => {
    cy.location().should(loc => {
      expect(loc.href).to.eq('https://dev.celuveat.com/restaurants/311?celebId=7');
    });
  });

  it('모바일에서 성시경, 소문난성수감자탕 페이지에서 로그인을 하고 로그아웃 할 시 비회원 상태로 돌아간다.', () => {
    cy.get('nav').find('button').last().click(); // 모바일 nav 하단의 마이 페이지 버튼을 누른다.

    cy.contains('로그아웃').click();

    cy.get('nav').find('button').last().click(); // 모바일 nav 하단의 마이 페이지 버튼을 다시 누른다.
