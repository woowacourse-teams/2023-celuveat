...
import static com.celuveat.restaurant.fixture.LocationFixture.검색_영역;
...
public class RestaurantAcceptanceTest extends AcceptanceTest {
    ...
    class 음식점_검색 {
        ...
        void 음식점_좋아요_기준_정렬() {
            ...
            var 오도 = 멤버("오도");
            var 로이스 = 멤버("로이스");
            var 오도_세션_아이디 = 회원가입과_로그인후_세션아이디를_가져온다(오도);
            var 로이스_세션_아이디 = 회원가입과_로그인후_세션아이디를_가져온다(로이스);
        ...
        void 음식점_검색_조건으로_검색한다() {
            ...
            // when
            var 응답 = 음식점_검색_요청(음식점_검색_조건_요청(말랑.id(), 없음, "말 랑 "), 검색_영역_요청(박스_1_2번_지점포함_요청));

            // then
            var 예상_응답 = 예상_응답(전체_음식점, 말랑.id(), 없음, "말 랑 ", 검색_영역(박스_1_2번_지점포함));
            조회_결과를_순서_상관없이_검증한다(예상_응답, 응답);
        }
    ...
    class 정보_수정_제안 {
        ...
        private TestData 여러_사용자가_음식점_좋아요를_누르고_조회한다() {
            ...
            var 오도_세션_아이디 = 회원가입과_로그인후_세션아이디를_가져온다(오도);
            var 로이스_세션_아이디 = 회원가입과_로그인후_세션아이디를_가져온다(로이스);
            var 도기_세션_아이디 = 회원가입과_로그인후_세션아이디를_가져온다(도기);
            var 말랑_세션_아이디 = 회원가입과_로그인후_세션아이디를_가져온다(말랑);
