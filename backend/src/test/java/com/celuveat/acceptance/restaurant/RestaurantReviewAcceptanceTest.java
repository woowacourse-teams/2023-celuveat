...
public class RestaurantReviewAcceptanceTest extends AcceptanceTest {
    ...
    void 음식점_리뷰를_작성한다() {
        ...
        var 요청 = 리뷰_요청("맛집이네요 또 올 것 같습니다");

        // when
        var 응답 = 리뷰_작성_요청을_보낸다(요청, 세션_아이디, 음식점.id());
    ...
    void 음식점_리뷰를_조회한다() {
        ...
        var 요청3 = 리뷰_요청("리뷰3");
        var 요청4 = 리뷰_요청("리뷰4");
        리뷰_작성_요청을_보낸다(요청1, 도기_세션_아이디, 음식점.id());
        리뷰_작성_요청을_보낸다(요청2, 로이스_세션_아이디, 음식점.id());
        리뷰_작성_요청을_보낸다(요청3, 말랑_세션_아이디, 음식점.id());
        리뷰_작성_요청을_보낸다(요청4, 말랑_세션_아이디, 음식점.id());
    ...
    void 다른사람의_음식점_리뷰를_수정한다() {
        ...
        // when
        var 응답 = 리뷰_수정_요청을_보낸다(리뷰_수정_요청, 도기_세션_아이디, 음식점.id(), 음식점_리뷰_아이디);
    ...
    @Test
    void 리뷰가_속한_음식점과_다른_음식점으로_수정요청을_했을때_잘못된_요청이라고_알려준다() {
        // given
        var 오도음식점 = 음식점("오도음식점");
        음식점을_저장한다(오도음식점);
        var 도기음식점 = 음식점("도기음식점");
        ...
        var 도기 = 멤버("도기");
        var 도기_세션_아이디 = 회원가입하고_로그인한다(도기);
        var 음식점_리뷰 = 음식점_리뷰(도기, 도기음식점);
        var 음식점_리뷰_아이디 = 음식점_리뷰를_저장한다(음식점_리뷰);
        var 리뷰_수정_요청 = 리뷰_수정_요청("리뷰 수정 요청");

        // when
        var 응답 = 리뷰_수정_요청을_보낸다(리뷰_수정_요청, 도기_세션_아이디, 오도음식점.id(), 음식점_리뷰_아이디);
    ...
    void 음식점_리뷰를_수정한다() {
        ...
        // when
        var 응답 = 리뷰_수정_요청을_보낸다(리뷰_수정_요청, 세션_아이디, 음식점.id(), 음식점_리뷰_아이디);
    ...
    void 음식점_리뷰를_삭제한다() {
        ...
        // when
        var 응답 = 리뷰_삭제_요청을_보낸다(세션_아이디, 음식점.id(), 음식점_리뷰_아이디);
