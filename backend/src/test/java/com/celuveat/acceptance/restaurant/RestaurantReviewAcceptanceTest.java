...
import static com.celuveat.acceptance.restaurant.RestaurantReviewAcceptanceSteps.신고_요청을_보낸다;
import static com.celuveat.acceptance.restaurant.RestaurantReviewAcceptanceSteps.음식점_리뷰_조회_응답을_검증한다;
import static com.celuveat.acceptance.restaurant.RestaurantReviewAcceptanceSteps.좋아요_요청을_보낸다;
...
public class RestaurantReviewAcceptanceTest extends AcceptanceTest {
    ...
    class 리뷰_API {
        ...
        void 음식점_리뷰를_작성한다() {
            ...
            // when
            var 응답 = 리뷰_작성_요청을_보낸다(말랑_세션_아이디, 요청);
        ...
        void 음식점_리뷰를_조회한다() {
            ...
            var 말랑_리뷰 = 리뷰_요청("말랑 리뷰", 대성집.id(), 4.9, List.of("mallang review image 1", "mallang review image 2"));
            var 도기_리뷰 = 리뷰_요청("도기 리뷰", 대성집.id(), 4.2);
            var 오도_리뷰 = 리뷰_요청("오도 리뷰", 대성집.id(), 3.3);
            var 로이스_리뷰 = 리뷰_요청("로이스 리뷰", 대성집.id(), 2.4, List.of("ro2s review image 1"));
            리뷰_작성_요청을_보낸다(말랑_세션_아이디, 말랑_리뷰);
            리뷰_작성_요청을_보낸다(도기_세션_아이디, 도기_리뷰);
            리뷰_작성_요청을_보낸다(오도_세션_아이디, 오도_리뷰);
            리뷰_작성_요청을_보낸다(로이스_세션_아이디, 로이스_리뷰);

            // when
            var 응답 = 리뷰_조회_요청을_보낸다(대성집.id());
        ...
        void 음식점_리뷰를_수정한다() {
            // given
            var 말랑_세션_아이디 = 로그인후_세션아이디를_가져온다(말랑);
            var 말랑_리뷰 = 리뷰_요청("말랑 리뷰", 대성집.id(), 4.9, List.of("mallang review image 1"));
            var 리뷰_ID = ID를_추출한다(리뷰_작성_요청을_보낸다(말랑_세션_아이디, 말랑_리뷰));
            var 리뷰_수정_요청 = 리뷰_수정_요청("리뷰 수정", 5.0);

            // when
            var 응답 = 리뷰_수정_요청을_보낸다(말랑_세션_아이디, 리뷰_ID, 리뷰_수정_요청);

            // then
            응답_상태를_검증한다(응답, 내용_없음);
            var 리뷰_조회_응답 = 리뷰_조회_요청을_보낸다(대성집.id());
        ...
        void 다른사람의_음식점_리뷰를_수정할_수_없다() {
            ...
            // when
            var 응답 = 리뷰_수정_요청을_보낸다(도기_세션_아이디, 리뷰_ID, 리뷰_수정_요청);
        ...
        void 음식점_리뷰를_삭제한다() {
            ...
            // when
            var 응답 = 리뷰_삭제_요청을_보낸다(말랑_세션_아이디, 리뷰_ID);
        ...
        void 다른_사람의_리뷰를_삭제할_수_없다() {
            ...
            // when
            var 응답 = 리뷰_삭제_요청을_보낸다(도기_세션_아이디, 리뷰_ID);
