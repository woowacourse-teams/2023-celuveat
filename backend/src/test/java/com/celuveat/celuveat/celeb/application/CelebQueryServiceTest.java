...
class CelebQueryServiceTest {
    ...
    private final FakeCelebQueryDao celebQueryDao = new FakeCelebQueryDao(videoDao);
    private final CelebQueryService celebQueryService = new CelebQueryService(celebQueryDao);
    ...
    @BeforeEach
    void setUp() {
        Long 히밥_ID = celebQueryDao.save(히밥());
        Long 성시경_ID = celebQueryDao.save(성시경());
        히밥 = 히밥(히밥_ID);
        성시경 = 성시경(성시경_ID);
    }
    ...
    class ID_로_셀럽_조회_시 {
        ...
        void 셀럽을_조회한다() {
            ...
            // when
            FindCelebByIdResponse response = celebQueryService.findById(히밥.id());
        ...
        void 셀럽이_없으면_예외이다() {
            ...
            BaseExceptionType baseExceptionType = assertThrows(CelebException.class, () ->
                    celebQueryService.findById(100L)
            ).exceptionType();
